public class LinkedList {
    ...
    private boolean compareData(Object listData, Object searchData) {
        ...
        if (listData.getClass().getSimpleName().equals("Contacto")) {
            try {
                
                java.lang.reflect.Field nombreField = listData.getClass().getDeclaredField("nombre");
                java.lang.reflect.Field telefonoField = listData.getClass().getDeclaredField("telefono");
                java.lang.reflect.Field direccionField = listData.getClass().getDeclaredField("direccion");
                
                nombreField.setAccessible(true);
                telefonoField.setAccessible(true);
                direccionField.setAccessible(true);
                
                String nombre = (String) nombreField.get(listData);
                String telefono = (String) telefonoField.get(listData);
                String direccion = (String) direccionField.get(listData);
                
                return nombre.toLowerCase().contains(searchStr) ||
                       telefono.toLowerCase().contains(searchStr) ||
                       direccion.toLowerCase().contains(searchStr) ||
